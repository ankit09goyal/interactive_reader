...
async function getSummaryStats(bookId) {
  const [totalTimeResult, sessionStats, uniqueSessions] = await Promise.all([
    ...
    // Session statistics
    ReadingAnalytics.aggregate([
      { $match: { bookId, eventType: { $in: ["session_start", "session_end"] } } },
      {
        $group: {
          _id: "$sessionId",
          start: { $min: { $cond: [{ $eq: ["$eventType", "session_start"] }, "$createdAt", null] } },
          end: { $max: { $cond: [{ $eq: ["$eventType", "session_end"] }, "$createdAt", null] } },
        },
      },
      {
        $project: {
...
async function getDropOffAnalysis(bookId) {
  ...
  const result = await ReadingAnalytics.aggregate([
    { $match: { bookId, eventType: { $in: ["page_view", "chapter_view", "time_update"] } } },
...
async function getHighlightsWithNotes(bookId) {
  const [totalHighlights, highlightsWithNotes] = await Promise.all([
    ...
    Highlight.countDocuments({ bookId, notes: { $ne: null, $ne: "" } }),
  ]);
  ...
  return {
    ...
    notesPercentage: totalHighlights > 0 ? Math.round((highlightsWithNotes / totalHighlights) * 100) : 0,
  };
}
...
async function getQuestionsByLocation(bookId) {
  ...
  return {
    ...
    summary: {
      ...
      answeredPercentage: totalQuestions > 0 ? Math.round((answeredQuestions / totalQuestions) * 100) : 0,
    },
  };
}
